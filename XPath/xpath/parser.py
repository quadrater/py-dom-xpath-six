...
class XPathScanner(Scanner):
    patterns = [
        ("r'\\:'", re.compile(u'\\:')),
        ("r'node\\s*\\('", re.compile(u'node\\s*\\(')),
        ("r'text\\s*\\('", re.compile(u'text\\s*\\(')),
        ("r'comment\\s*\\('", re.compile(u'comment\\s*\\(')),
        ("r'processing-instruction\\s*\\('", re.compile(u'processing-instruction\\s*\\(')),
        ("r'\\,'", re.compile(u'\\,')),
        ("r'\\.'", re.compile(u'\\.')),
        ("r'\\$'", re.compile(u'\\$')),
        ("r'\\)'", re.compile(u'\\)')),
        ...
        ("r'\\-'", re.compile(u'\\-')),
        ("'\\|'", re.compile(u'\\|')),
        ("r'and'", re.compile(u'and')),
        ("r'or'", re.compile(u'or')),
        (u'\\s+', re.compile(u'\\s+')),
        ('END', re.compile(u'$')),
        ('FORWARD_AXIS_NAME', re.compile(u'child|descendant-or-self|attribute|self|descendant|following-sibling|following|namespace')),
        ('REVERSE_AXIS_NAME', re.compile(u'parent|preceding-sibling|preceding|ancestor-or-self|ancestor')),
        ('NCNAME', re.compile(u'[a-zA-Z_][a-zA-Z0-9_\\-\\.\\w]*(?!\\()')),
        ('FUNCNAME', re.compile(u'[a-zA-Z_][a-zA-Z0-9_\\-\\.\\w]*')),
        ('DQUOTE', re.compile(u'\\"(?:[^\\"])*\\"')),
        ('SQUOTE', re.compile(u"\\'(?:[^\\'])*\\'")),
        ('NUMBER', re.compile(u'((\\.[0-9]+)|([0-9]+(\\.[0-9]*)?))([eE][\\+\\-]?[0-9]+)?')),
        ('EQ_COMP', re.compile(u'\\!?\\=')),
        ('REL_COMP', re.compile(u'[\\<\\>]\\=?')),
        ('ADD_COMP', re.compile(u'[\\+\\-]')),
        ('MUL_COMP', re.compile(u'\\*|div|mod')),
    ]
    def __init__(self, str):
        Scanner.__init__(self,None,[u'\\s+'],str)
